...
rp <- function() {
  ...
  axis(
    2, c(log(100), log(250), log(500), log(1000), log(2000)),
  ...
  plot(tdat,
    xlim = c(70, 0), ylim = c(-5, 34), axes = FALSE,
...
plot(cp.c[, 3], tp.c[, 2],
  xlim = range(cp.c[, -1]), ylim = range(tp.c[, -1]),
...
arrows(cp.c[, 2], tp.c[, 3], cp.c[, 4], tp.c[, 3],
  length = 0,
  lwd = 0.75, col = "grey40"
)
arrows(cp.c[, 3], tp.c[, 2], cp.c[, 3], tp.c[, 4],
  length = 0,
  lwd = 0.75, col = "grey40"
)
lines(cp.c[, 3], tp.c[, 3], lwd = 2, col = "grey40")
points(cp.c[, 3], tp.c[, 3],
  pch = 21, bg = cols[ci],
  col = "grey40", cex = 1.25
)

arrows(tring$C_025[-8], tring$T_5[-8], tring$C_975[-8], tring$T_5[-8],
  length = 0,
  lwd = 0.75
)
arrows(tring$C_5[-8], tring$T_025[-8], tring$C_5[-8], tring$T_975[-8],
  length = 0,
  lwd = 0.75
)
...
text(cp.c[13, 3], tp.c[13, 3] - 0.3, "60",
  pos = 2,
  col = "grey40"
)
text(cp.c[33, 3], tp.c[33, 3], "50",
  pos = 3, offset = 0.7,
  col = "grey40"
)
text(cp.c[53, 3], tp.c[53, 3] - 0.3, "40",
  pos = 4, offset = 1,
  col = "grey40"
)
text(cp.c[73, 3], tp.c[73, 3], "30",
  pos = 1, offset = 1,
  col = "grey40"
)
text(cp.c[93, 3] + 0.1, tp.c[93, 3], "20",
  pos = 3, offset = 1.2,
  col = "grey40"
)
text(cp.c[113, 3], tp.c[113, 3], "10",
  pos = 2, offset = 2.7,
  col = "grey40"
)

text(1.52, -2, "5 \u00B0C/doubling",
  col = "white",
  srt = (sin(4.8 / (diff(par("usr")[3:4]) / diff(par("usr")[1:2])))) / pi * 180
)
text(0.95, -1.8, "8 \u00B0C/doubling",
  col = "white",
...
axis(1,
  at = (log(c(250, 500, 1000, 1500)) - log(280)) / log(2),
...
plot(0, 0,
  type = "n", xlim = c(65, 0), ylim = c(0, 1), axes = FALSE,
  xlab = "", ylab = ""
)
### Warming stripes
for (i in 7:nrow(tp)) {
  polygon(c(rep(tp$ages[i] - 0.25, 2), rep(tp$ages[i] + 0.25, 2)),
    c(0, 1, 1, 0),
    col = wscols[tcol[i]], border = NA
  )
}
### CO2
polygon(c(cp.trunc$ages, rev(cp.trunc$ages)),
  c(
    (cp.trunc$X2.5. - min(cp.trunc[, -1])) / diff(range(cp.trunc[, -1])),
...
ticks <- c(
  (log(270) - min(cp.trunc[, -1])) / diff(range(cp.trunc[, -1])) * 0.9 + 0.05,
...
polygon(c(legx.min, legx.min, legx.max, legx.max),
  c(legy.min, legy.max, legy.max, legy.min),
  lwd = 3,
...
tvals <- exp(tvals) - 10
for (i in 1:4) {
  lines(rep(tticks[i], 2), c(legy.min, legy.min - 0.025),
    col = "white",
    lw = 1.5, lend = 1
  )
  text(
    x = tticks[i], y = legy.min - 0.055, round(tvals[i], 1), col = "white",
    cex = 0.7
  )
}
text(mean(tticks), legy.min - 0.11,
  expression(Delta * " GMST (K relative to preindustrial)"),
  cex = 0.7,
  col = "white"
)
dev.off()
