...
points(dat$pco2.age, exp(dat$pco2),
  cex = 0.5,
...
plot(tdat,
  xlim = c(65, 0), ylim = c(-20, 22), axes = FALSE,
...
legend(67, log(300), c("1 Myr bin", "5 Myr bin"),
  lty = c(1, 3),
...
par(mar = c(5, 5, 1, 1))
plot(a1, np1,
  xlim = c(67, 0), type = "l", cex.axis = 1.5,
...
legend(67, 5.4, c("1 Myr bin", "5 Myr bin"),
  lty = c(1, 3),
  lwd = c(2, 2), bty = "n", cex = 1.5,
  col = rgb(112, 173, 71, maxColorValue = 255)
)
...
plot(dat$pco2.age[le.ind], dat$pco2[le.ind],
  xlim = c(55, 25), ylim = c(5.5, 9.5),
  pch = 21, bg = "dark gray", axes = FALSE, xlab = ("Age(Ma)"),
  ylab = ""
)
arrows((dat$pco2.age - dat$pco2.age.sd)[le.ind], dat$pco2[le.ind],
  (dat$pco2.age + dat$pco2.age.sd)[le.ind], dat$pco2[le.ind],
  length = 0
)
arrows(dat$pco2.age[le.ind], (dat$pco2 - dat$pco2.sd)[le.ind],
  dat$pco2.age[le.ind], (dat$pco2 + dat$pco2.sd)[le.ind],
  length = 0
)
...
## Figure
png("out/other_figs/UU_press.png", width = 6, height = 2.9, units = "in", res = 600)
...
## Print figure
png("out/other_figs/Lamont_press.png", width = 6, height = 2.9, units = "in", res = 600)
par(mai = c(0.5, 0, 0, 0.6))
plot(0, 0,
  type = "n", xlim = c(65, 0), ylim = c(0, 1), axes = FALSE,
  xlab = "", ylab = ""
